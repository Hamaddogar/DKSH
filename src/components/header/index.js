...
const searchButton = (searchDropdown, dark) => {
 if (searchDropdown) return <img src={SubtractIcon} style={{ marginTop: "5px", width: "20px" }} />;
    ...
    {users.map((user, k) => (
     <div className="dropdown-content" key={k}>
      <div className="dis-flex">
       {CustomIcon(<img width={25} src={user.profileImg} style={{ borderRadius: "50%", background: "none" }} />)}
       <label className="searched-username"> {user.username}</label>
      </div>
      <div className="dis-flex" style={{ gap: 20 }}>
       <div className="dis-flex">
        <img src={StarIcon} width={22} />
        <label className={dark ? "label label-dark" : "label"}> {user.rating} / 5</label>
       </div>
       <div className="dis-flex">
        <img src={!dark ? MsgIcon : Images.IconDarkLight} width={21} height={19} />
  ...
  <div className={dark ? "header-container header-container-dark " : "header-container"}>
   <Grid container spacing={2}>
    <Grid item xs={10.5} sm={2} md={1.5} lg={1.7} sx={{ display: { sm: "none", md: "block" } }}>
     <img src={dark ? Images.WhiteLogo : Images.Logo} className="header-logo" />
    </Grid>
    ...
    >
     <div className="profile-container">
      <div className={dark ? "header-Profile-Name header-Profile-Name-dark" : "header-Profile-Name"}>John Yev</div> <img src={Images.Profile} className="header-Profile-icon" />
      <div>{ArrowIcon()}</div>
     </div>
    </Grid>
   </Grid>

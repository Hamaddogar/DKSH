...
const Index = ({ user, dark }) => {
    ...
    return (
        <div
            className={
                dark
                    ? 'banner-container banner-container-dark  user-network'
                    : 'banner-container user-network'
            }
        >
            <div className="user-network-container">
                <div className="network-user-profile">
                    ...
                    <div className="user-profile-content">
                        <h1 className={dark ? 'job-title white-text' : 'job-title'}>
                            {`${user?.firstName} ${user?.lastName}`}{' '}
                        </h1>
                        <h3
                            className={
                                dark ? 'network-user-reviews white-text' : 'network-user-reviews'
                            }
                            style={{ margin: 0 }}
                        >
                            <img src={Images.Star} alt="" />
                            {ratingCount(user?.comments, commentCount(user?.comments))} (
                            {commentCount(user?.comments)} Reviews)
                        </h3>
                        <p
                            className={dark ? 'network-user-exp white-text' : 'network-user-exp'}
                            style={{ width: '90%' }}
                        >
                            {user?.description}
                        </p>

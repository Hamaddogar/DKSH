...
const SMFilters = ({ open, onClose = () => {}, condition, dark }) => {
    ...
    const ServiceDropdown = () => {
        const handleChangeService = (service) =>
            dispatch(ALPHA_FUNCTIONS.SERVICE_FUNCTION(service));
        return (
            <Box className="dropdown-list" sx={{ gap: '10px' }}>
                {condition &&
                    servicesFilterOptions.map((service, key) => (
                        <Box
                            onClick={() => handleChangeService(service)}
                            key={service}
                            className={
                                (condition ? servicesFilter : servicesFilterJOB) === service
                                    ? dark
                ...
                {!condition &&
                    servicesFilterOptionsJOB.map((service, key) => (
                        <Box
                            onClick={() => handleChangeService(service)}
                            key={service}
                            className={
    ...
    const RatingDropdown = () => {
        ...
        return (
            <Box className="dropdown-list" sx={{ gap: '10px' }}>
                {list.map((option) => (
                    <Box
                        onClick={() => handleChangeRating(option)}
                        key={option}
                        className={
                            (condition ? ratingFilter : ratingFilterJOB) === option
                                ? dark
                                    ? 'dropdown-active-item white-text'
                                    : 'dropdown-active-item'
                                : 'dropdown-item'
    ...
    const handleAvailableToWork = (event) =>
        dispatch(ALPHA_FUNCTIONS.AVALIABLE_FUNCTION(event.target.checked));
    const handleProTallent = (event) =>
        dispatch(ALPHA_FUNCTIONS.PRO_FUNCTION(event.target.checked));
            ...
            <Box sx={{ px: '16px', pt: 5, mb: 17 }}>
                <div className="sm-filters-title">
                    ...
                    {dark ? (
                        ...
                        <img alt="" src={CrossIcon} onClick={onClose} />
                    )}
                </div>
                <CustomButton
                    children={
                ...
                {condition && (
                    <>
                        ...
                        <ToggleButton
                            ...
                            onChange={handleAvailableToWork}
                            checked={availableToWorkFilter}
                            dark={dark}
                        />
                        <br />
                        <ToggleButton
                            onChange={handleProTallent}
                            checked={proTallentFilter}
                            label="Pro tallent"
                            dark={dark}
                        />
                    </>
                ...
                >
                    <Box>
                        <Button
                            ...
                            type="secondary"
                            startIcon={<img alt="" src={CrossIcon} />}
                        />
                    </Box>
                    <Box>
                        <Button
                            ...
                            title="Apply filter"
                            type="secondary"
                            startIcon={<img alt="" src={CheckIcon} />}
                        />
                    </Box>
                </Box>
            </Paper>
...
const ToggleButton = ({ label, dark, ...rest }) => {
    return (
        <Box sx={{ ...styleSheet.button, ...styleSheet.flex }}>
            <span className={dark ? 'switch-label switch-label-dark' : 'switch-label'}>
                {label}
            </span>
            <SwitchBase {...rest} />
        </Box>

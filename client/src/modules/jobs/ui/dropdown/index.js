...
function DropdownButton({ icon, label, dark, value, children, onClick }) {
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (
                ...
                event.target.tagName !== specificNode.tagName
            ) {
                setIsOpen(false);
            }
        }
    ...
    return (
        <div className="custom-dropdown" ref={dropdownRef}>
            <Box
                sx={{ ...styleSheet.filterButton, ...styleSheet.flex }}
                style={{ gap: 18 }}
                onClick={() => setIsOpen(!isOpen)}
            >
                <Box
                    onClick={onClick}
                    sx={{ ...styleSheet.flex, justifyContent: 'flex-start' }}
                    style={{ gap: 8.8 }}
                >
            ...
            {isOpen && label !== 'Reset all' && (
                <Box
                    sx={{
                        ...styleSheet.dropdownContainer,
                        background: dark ? '#0D1013' : '#FFFFFF',
                    }}
                >
                    {children}
                </Box>

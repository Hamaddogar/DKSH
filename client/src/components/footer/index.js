...
const Index = ({ dark }) => {
    ...
    const color = (href) =>
        !dark
            ? `${path === href ? '#8077F6' : '#090b0c'}`
            : `${path === href ? '#8077F6' : '#fff'}`;
    return (
        <Paper
            sx={{
                zIndex: 99999,
        ...
        >
            <BottomNavigation
                showLabels
                sx={{ paddingTop: '10px', background: dark ? '#090b0c' : '#fff' }}
            >
                {links.map((link) => (
                    <BottomNavigationAction
                        key="botNav"
                        onClick={handleClearNavigation(link.href)}
                        sx={{ gap: '14px' }}
                        label={<label style={{ color: color(link.href) }}>{link.label}</label>}
                        icon={link.icon}
                    />
                ))}
            </BottomNavigation>
...
const HomeButtonIcon = (dark, path) => {
    if (path === '/') return <img alt="" src={ActiveHome} />;
...
const networkButton = (dark, path) => {
    if (path === '/network') return <img alt="" src={ActiveNetwork} />;
...
const JobsIcon = (dark, path) => {
    if (path === '/jobs') return <img alt="" src={ActiveJob} />;

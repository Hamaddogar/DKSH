...
import { commentCount, formatDate, profileName, ratingCount } from "../../utils/HELPER";
  ...
  return (
    <svg width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        fill="#8077F6"
      />
      <path d="M8.64282 11.2679L10.6071 13.2322L14.3571 8.85718" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
};
    ...
    <>
      {view === "List" ? (
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={7} md={6} lg={4}>
              <Grid container spacing={2}>
                ...
                <Grid item sm={9}>
                  <div className={dark ? "List-profile-name List-profile-name-dark" : "List-profile-name"}>{profileName(user?.firstName, user?.lastName)}</div>
                  ...
                  <div className="commentRating-container">
                    <div className={dark ? "commentsRating-Listcount commentsRating-Listcount-dark" : "commentsRating-Listcount"}>
                      <div className="margin-icon">{commentIcon(dark)}</div> {commentCount(user?.comments)} <span className="commentsRating-Listtitle">Comments</span>
                    </div>
                    <div className={dark ? "commentsRating-Listcount commentsRating-Listcount-dark" : "commentsRating-Listcount"}>
                      {ratingCount(user?.comments, commentCount(user?.comments))} / 5 <span className="commentsRating-Listtitle">Rating</span>
                    </div>
                  </div>
                </Grid>
              </Grid>
        ...
        <div onClick={handleClickOpen("paper")} className={dark ? "card-grid-container card-grid-container-dark" : "card-grid-container"}>
          ...
          <div className={dark ? "grid-profile-name grid-profile-name-dark " : "grid-profile-name"}>{profileName(user?.firstName, user?.lastName)}</div>
          ...
          <div className="commentRating-container">
            <div className={dark ? "commentsRating-count commentsRating-count-dark" : "commentsRating-count"}>
              <div className="margin-icon">{commentIcon(dark)}</div> {commentCount(user?.comments)} <span className="commentsRating-title">Comments</span>
            </div>
            <div className={dark ? "commentsRating-count commentsRating-count-dark" : "commentsRating-count"}>
              {ratingCount(user?.comments, commentCount(user?.comments))} / 5 <span className="commentsRating-title">Rating</span>
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            <DialogContentText sx={{ mb: { xs: 8, md: 0 } }}>
              ...
              <div className={dark ? "dialog-profile-name dialog-profile-name-dark " : "dialog-profile-name"}>{profileName(user?.firstName, user?.lastName)}</div>
              <div className={dark ? "rating-Container rating-Container-dark " : "rating-Container"}>
                {ratingCount(user?.comments, commentCount(user?.comments))}<span className="timehoursList-title"> / 5</span>
              </div>
              <div className={dark ? "Hr-grid Hr-grid-dark" : "Hr-grid"}></div>
              <div className={dark ? "Comments-dialogTitle Comments-dialogTitle-dark" : "Comments-dialogTitle"}>
                Comments <span className={dark ? "container-commentsCount Comments-dialogTitle-dark" : "container-commentsCount"}> {commentCount(user?.comments)}</span>
              </div>
